...
class PVPatientFavoriteDoctors extends ChangeNotifier {
  List<ClsClinicDoctorDto>? favoriteDoctors;
  ...
  Future<void> getPatientFavoriteDoctors({
    int pageSize = 6,
    ...
    final result = await DoctorConnect.getPatientFavoriteDoctors(
      patientID,
      pageSize,
      _currentPage,
    );
      ...
      (right) {
        if (right.isEmpty || right.length < pageSize) {
          if (favoriteDoctors != null) {
            favoriteDoctors!.addAll(right);
          } else {
            favoriteDoctors = right;
          }

          _isFinished = true;
        } else {
          favoriteDoctors = [...favoriteDoctors ?? [], ...right];
          _currentPage++;
          _isLoaded = true;
        }
        notifyListeners();
